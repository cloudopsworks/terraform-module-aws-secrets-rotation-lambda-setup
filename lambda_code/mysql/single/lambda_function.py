...
def set_secret(service_client, arn, token):
    ...
    if conn:
        conn.close()
        logger.info("setSecret: AWSPENDING secret is already set as password in MySQL DB for secret arn %s." % scrub(arn))
        return
    ...
    try:
        with conn.cursor() as cur:
            ...
            ver = cur.fetchone()
            # Each branch is a fixed statement with the password bound as a
            # parameter, so the credential is never concatenated into query text.
            if ver[0].startswith("8"):
                cur.execute("SET PASSWORD = %s", (pending_dict['password'],))
            else:
                cur.execute("SET PASSWORD = PASSWORD(%s)", (pending_dict['password'],))
            conn.commit()
            logger.info("setSecret: Successfully set password for user %s in MySQL DB for secret arn %s." % (scrub(pending_dict['username']), scrub(arn)))
    finally:
        conn.close()
...
def test_secret(service_client, arn, token):
    ...
    if conn:
        ...
        logger.info("testSecret: Successfully signed into MySQL DB with AWSPENDING secret in %s." % scrub(arn))
        return
    else:
...
def generate_connection_string(secret_dict, new_password):
    """Generates a connection string for the MySQL database
    ...
    Supported values of secret_dict['connection_string_type'] are:
        - jdbc: MySQL Connector/J URL, 'jdbc:mysql://host:port/db?user=...'
        - dotnet: MySqlConnector keyword string, 'Server=...;Port=...;'
        - odbc: MySQL ODBC keyword string, 'Driver={MySQL ODBC 8.0 Unicode Driver};...'
        - go | gomysql: go-sql-driver/mysql DSN, 'user:password@tcp(host:port)/db'
        - uri | node | nodejs | python | pymysql: URI, 'mysql://user:password@host:port/db'
    ...
    if dialect == 'jdbc':
        query = build_query_string([
            ...
            ('sslMode', 'REQUIRED' if use_ssl else 'DISABLED'),
        ])
        return "jdbc:mysql://%s:%s/%s?%s" % (host, port, dbname, query)
    ...
    if dialect == 'odbc':
        return "Driver={MySQL ODBC 8.0 Unicode Driver};" + build_property_string([
            ('Server', host),
            ('Port', port),
            ('Database', dbname),
            ('User', username),
    ...
    if dialect == 'go':
        # The go-sql-driver/mysql DSN is not a URL: the user name and password are
    ...
    # Generic URI, accepted by mysql2, SQLAlchemy and the other URL based clients.
    # TLS is left to the client here: the drivers disagree on how to spell it in a
    # URI, and emitting one dialect's spelling would break the others.
    return "mysql://%s@%s:%s/%s" % (build_userinfo(username, new_password), host, port, dbname)
